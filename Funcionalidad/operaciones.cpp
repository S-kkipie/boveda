...
void depositar(EntidadBancaria* banco, Boveda* boveda, ElementoMonetario* elemento, int cantidad, Transportadora* transportadora) {
    ...
    if (estado == Estado::Dañado) throw ExcepcionElementoMonetarioInvalido("Dinero dañado detectado");
...
void retirar(EntidadBancaria* banco, Boveda* boveda, ElementoMonetario* elemento, int cantidad, Transportadora* transportadora) {
    ...
    double saldo = 0;
    for (auto& p : boveda->getPaquetesMonetarios()) {
...
void transferir(
    ...
    if (!plazaCoincide)
        throw ExcepcionOperacionInvalida("Transportadora no operativa en las plazas especificadas");
    ...
    if (hayDaniado) {
        for (auto* p : paquetesTransferidos) delete p;
        throw DineroDaniadoException("Dinero dañado detectado en transferencia. No se puede completar la operación.");
    }

    if (dis(gen) < 20) throw TransporteDemoradoException("Transporte demorado");
